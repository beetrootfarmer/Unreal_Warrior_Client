...
function webRtcPlayer(parOptions) {
        ...
        return function (stats) {
            ...
            stats.forEach(stat => {
                ...
                if (
                    ...
                    if (
                        self.aggregatedStats &&
                        self.aggregatedStats.timestamp
                    ) {
                        // Get the mimetype of the video codec being used
                        if (
                            stat.codecId &&
                            self.aggregatedStats.codecs &&
                            self.aggregatedStats.codecs.hasOwnProperty(
                                stat.codecId
                            )
                        ) {
                            newStat.videoCodec =
                                self.aggregatedStats.codecs[stat.codecId];
                        }
                        ...
                        if (self.aggregatedStats.framesDecoded) {
                            ...
                            newStat.framerate =
                                (newStat.framesDecoded -
                                    self.aggregatedStats.framesDecoded) /
                                ((newStat.timestamp -
                                    self.aggregatedStats.timestamp) /
                                    1000);
                    ...
                    if (
                        ...
                        if (self.aggregatedStats.audioBytesReceived) {
                            newStat.audioBitrate =
                                ...
                                (stat.timestamp -
                                    self.aggregatedStats.timestamp);
                            newStat.audioBitrate = Math.floor(
                                newStat.audioBitrate
                            );
                        }
                    }
                    ...
                    self.pcClient
                        ...
                        .then(() => {
                            if (self.onWebRtcAnswer) {
                                self.onWebRtcAnswer(
                                    self.pcClient.currentLocalDescription
                                );
                            }
                        })
                        .then(() => {
    ...
    this.receiveSFUPeerDataChannelRequest = function (channelData) {
        ...
        const unidirectional =
            channelData.sendStreamId != channelData.recvStreamId;
